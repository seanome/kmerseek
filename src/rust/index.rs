...
mod tests {
    ...
    #[test]
    fn test_manual_vs_existing_auto_index_equivalence() {
        // Define paths
        ...
        // Create index with manual path
        let manual_index = ProteomeIndex::new(index_dir.clone(), 16, 5, "hp", SEED, false).unwrap();
        ...
        // Load the existing index that was created at the autogenerated filename path
        println!("Loading existing auto-generated index...");
        let auto_index_filename = format!("bcl2_first25_uniprotkb_accession_O43236_OR_accession_2025_02_06.fasta.gz.hp.k16.scaled5.kmerseek.rocksdb");
        let auto_index_path = fasta_path.parent().unwrap().join(auto_index_filename);
        assert!(
            auto_index_path.exists(),
            "Auto-generated index not found at {:?}",
            auto_index_path
        );

        let auto_index = ProteomeIndex::load(&auto_index_path).unwrap();

        // Print loaded auto index stats
        println!("Loaded auto-generated index stats:");
        auto_index.print_stats();

        // Verify the loaded auto index has the same content
        assert!(auto_index.signature_count() == 25, "Loaded auto index should have 25 signatures");
        assert!(
            auto_index.combined_minhash_size() == 1603,
            "Loaded auto index should have combined minhash of size 1603"
        );
        ...
        assert!(are_equivalent, "Manual and loaded auto-generated indices should be equivalent");
